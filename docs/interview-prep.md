...
You: My approach would be to count the frequency of each character. If the two frequency maps are identical, they are anagrams. Let me write the code...
You: Time is O(n) because we iterate once. Space is O(n) in the worst case, but since there are only 26 lowercase letters, it is O(1).
